...
{
    public delegate void AddToPropagator(Neighbour<T> neighbour, int[][,] actionMaps, int[][] orientedTileIds , bool[][][] propagator, int action, Direction direction);
...
public class TilingWFC<T> : Model
{
    ...
    */
    ...
    {
        if (tileId >= _orientedTileIds.Length || orientation >= _orientedTileIds[tileId].Length || y >= waveHeight ||
            x >= waveWidth)
        {
            return false;
        }
    ...
    (bool, T[,]) Run()
    {
        (bool Success, int[,] Result) a = base.Run();
        if (a.Success == false)
        {
            return (false, null);
        }
